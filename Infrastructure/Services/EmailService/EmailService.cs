...
public class EmailService(EmailConfiguration configuration, ILogger<EmailService> logger) : IEmailService
{
    ...
    {
        logger.LogInformation("Starting method SendEmail in time {DateTime}", DateTimeOffset.UtcNow);

        var emailMessage = CreateEmailMessage(message, format);
        await SendAsync(emailMessage);

        logger.LogInformation("Finished method SendEmail in time {DateTime}", DateTimeOffset.UtcNow);
    }
    ...
    private MimeMessage CreateEmailMessage(EmailMessageDto message, TextFormat format)
    {
        ...
        logger.LogInformation("Finished method CreateEmailMessage in time {DateTime}", DateTimeOffset.UtcNow);
        return emailMessage;
    }
    ...
    private async Task SendAsync(MimeMessage mailMessage)
    {
        ...
        try
        {
            logger.LogInformation("Starting method SendAsync in time {DateTime}", DateTimeOffset.UtcNow);
        ...
        finally
        {
            ...
            client.Dispose();
            logger.LogInformation("Finished method SendAsync in time {DateTime}", DateTimeOffset.UtcNow);
        }
    }
