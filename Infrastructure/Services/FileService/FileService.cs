...
public class FileService(IWebHostEnvironment hostEnvironment, ILogger<FileService> logger) : IFileService
{
    ...
    public async Task<string> CreateFile(IFormFile file)
    {
        try
        {
            logger.LogInformation("Starting method CreateFile in time:{DateTime} ", DateTimeOffset.UtcNow);
            var fileName =
            ...
            logger.LogInformation("Finished method CreateFile in time:{DateTime} ", DateTimeOffset.UtcNow);
            return fileName;
        }
        catch (Exception e)
        {
    ...
    public bool DeleteFile(string file)
    {
        try
        {
            logger.LogInformation("Starting method DeleteFile in time:{DateTime} ", DateTimeOffset.UtcNow);
            var fullPath = Path.Combine(hostEnvironment.WebRootPath, "images", file);
            File.Delete(fullPath);
            logger.LogInformation("Finished method DeleteFile in time:{DateTime} ", DateTimeOffset.UtcNow);
            return true;
        }
        catch (Exception e)
        {
