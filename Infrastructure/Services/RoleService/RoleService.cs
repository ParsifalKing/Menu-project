...
public class RoleService(ILogger<RoleService> logger, DataContext context) : IRoleService
{
    ...
    public async Task<Response<GetRoleDto>> GetRoleByIdAsync(Guid roleId)
    {
        try
        {
    ...
    public async Task<Response<string>> CreateRoleAsync(CreateRoleDto createRole)
    {
        try
        {
            logger.LogInformation("Starting method CreateRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);
            ...
            logger.LogInformation("Finished method CreateRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);
    ...
    public async Task<Response<string>> UpdateRoleAsync(UpdateRoleDto updateRole)
    {
        try
        {
            logger.LogInformation("Starting method UpdateRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);
            var existingRole = await context.Roles.AnyAsync(x => x.Name == updateRole.Name);
            if (existingRole)
            {
                logger.LogWarning("Role already exists by name:{Name},time:{Time}", updateRole.Name,
                    DateTimeOffset.UtcNow);
                return new Response<string>(HttpStatusCode.BadRequest,
                    $"Already exists role by name:{updateRole.Name}");
            }
            ...
            logger.LogInformation("Finished method UpdateRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);
    ...
    public async Task<Response<bool>> DeleteRoleAsync(Guid roleId)
    {
        try
        {
            logger.LogInformation("Starting method DeleteRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);

            var role = await context.Roles.Where(x => x.Id == roleId).ExecuteDeleteAsync();

            logger.LogInformation("Finished method DeleteRoleAsync in time:{DateTime} ", DateTimeOffset.UtcNow);
            return role == 0
