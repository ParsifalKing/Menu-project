...
public class HashService(ILogger<HashService> logger) : IHashService
{
    ...
    public string ConvertToHash(string rawData)
    {
        try
        {
            logger.LogInformation("Starting method ConvertToHash in time {DateTime}", DateTimeOffset.UtcNow);
            ...
            logger.LogInformation("Finished method ConvertToHash in time {DateTime}", DateTimeOffset.UtcNow);
